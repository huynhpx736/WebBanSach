...
const CreateOrder = () => {
  ...
  const handleProvinceChange = async (e) => {
    ...
    // setServices([]);
    // setSelectedService(null);
    // setReceiverAddress(selectedProvince.data.ProvinceName);
    //set receiver address = province
    setReceiverAddress(provinces.find(p => p.ProvinceID === selectedProvince)?.ProvinceName);
  };
  ...
  const handleDistrictChange = async (e) => {
    ...
    // setServices([]);
    // setSelectedService(null);
    //set receiver address = district + province
    setReceiverAddress(`${districts.find(d => d.DistrictID === selectedDistrict)?.DistrictName}, ${provinces.find(p => p.ProvinceID === selectedProvince)?.ProvinceName}`);
  };
  ...
  const handleWardChange = async (e) => {
    ...
    // const servicesData = await fetchAvailableServices(token, shopId, DistricIDFrom, selectedDistrict);
    // setServices(servicesData.data);
    //set receiver address = ward + district + province
    setReceiverAddress(`${wards.find(w => w.WardCode === selectedWard)?.WardName}, ${districts.find(d => d.DistrictID === selectedDistrict)?.DistrictName}, ${provinces.find(p => p.ProvinceID === selectedProvince)?.ProvinceName}`);
  };
  ...
  const handlePlaceOrder = async () => {
    ...
    // const receiverAddress = `${wards.find(w => w.WardCode === selectedWard)?.WardName}, ${districts.find(d => d.DistrictID === selectedDistrict)?.DistrictName}, ${provinces.find(p => p.ProvinceID === selectedProvince)?.ProvinceName}`;
                      const receiverAddress = `${detailAddress}, ${wards.find(w => w.WardCode === selectedWard)?.WardName || ''}, ${districts.find(d => d.DistrictID === selectedDistrict)?.DistrictName || ''}, ${provinces.find(p => p.ProvinceID === selectedProvince)?.ProvinceName || ''}`;
