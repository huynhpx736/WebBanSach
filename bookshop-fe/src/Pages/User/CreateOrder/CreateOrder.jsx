...
const CreateOrder = () => {
  ...
  const handleProvinceChange = async (e) => {
    ...
    const data = await fetchDistricts(token, province);
  ...
  const handleDistrictChange = async (e) => {
    ...
    const data = await fetchWards(token, district);
    setWards(data.data);
    setSelectedWard(null);
    setShippingFee(0);
  };
  ...
  useEffect(() => {
    ...
    if (selectedProvince && selectedDistrict && selectedWard) {
      ...
      setReceiverAddress(`${detailAddress}, ${wardName}, Huyện Vân Canh, Tỉnh Bình Định`);
    ...
    // Lặp qua các params và gọi API calculateShippingFee
    const tryShippingFee = async (paramsList) => {
      for (const param of paramsList) {
        const params = { ...commonParams, ...param }; // kết hợp params chung với service_id
        try {
          const data = await calculateShippingFee(token, params);
          //nếu chi phí vận chuyển lớn hơn 1000000 thì thông báo chi phí vận chuyển quá lớn không thể giao hàng
          setShippingFee(data.data.total);
          if (data.data.total > 400000) {
            setError('Chi phí vận chuyển quá lớn không thể giao hàng.');
            return;
          }
          
          return; // Thoát vòng lặp nếu gọi API thành công
        } catch (error) {
          // Tiếp tục thử với tham số tiếp theo nếu có lỗi

        }
      }
      setError('Địa chỉ không nằm trong khu vực giao hàng.');
    };

    // Gọi hàm thử tất cả các params
    tryShippingFee(paramsList);
  }
...
//                   {service.map(s => (
//                     <option key={s.service_id} value={s}>{s.short_name}</option>
//                   ))}
//                 </select>
//               </div>
